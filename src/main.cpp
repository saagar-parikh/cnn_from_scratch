...
int main(int argc, char **argv)
{
    ...
    // initialize fc layer
    vector<Tensor<double>> fc1_weights_and_biases = fc_init_weights(input_size, 2, 0);
    // vector<Tensor<double>> fc2_weights_and_biases = fc_init_weights(512, output_size, 42);
    ...
    for (int i = 0; i < num_test_batches; ++i)
    {
        ...
        err = cudaMalloc(&mat2, fc1_weights.dims[0] * fc1_weights.dims[1] * sizeof(float));
        if (err != cudaSuccess)
        {
        ...
        err = cudaMalloc(&out, input.dims[0] * fc1_weights.dims[1] * sizeof(float));
        if (err != cudaSuccess)
        {
        ...
        cudaMemcpy(mat2,
                   fc1_weights.data_,
                   fc1_weights.dims[0] * fc1_weights.dims[1] * sizeof(float),
                   cudaMemcpyHostToDevice);
